...
//koneksi database
  ...
  if(isset($_POST['bsimpan']))
  {
    ...
    {
      //edit
      $edit = mysqli_query($koneksi, "UPDATE kap set
                        Id = '$_POST[Id]',
    ...
    {
      //new
      $simpan = mysqli_query($koneksi, "INSERT INTO kap (Id, Nama_Mahasiswa, Jurusan)
                      VALUES ('$_POST[Id]',
  ...
  if(isset($_GET['hal']))
  {
    ...
    if ($_GET['hal'] == "hapus")
     {
      //hapus
      $hapus = mysqli_query($koneksi, "DELETE  FROM kap WHERE Id = '$_GET[Id]' ");
      if($hapus) 
      {
        echo "<script>
  ...
  if(isset ($_GET['hal']))
  {
    ...
    if ($_GET['hal'] == "edit")
    {
        $tampil = mysqli_query($koneksi, "SELECT * FROM kap WHERE Id = '$_GET[Id]' ");
        $data = mysqli_fetch_array($tampil);
        if($data)
        {
            $vId = $data['Id'];
            $vNama_Kry = $data['Nama_Mahasiswa'];
            $vJabatan = $data['Jurusan'];
        }
    }
  }
?>
...
<body>
  ...
  <div class="row no-gutters mt-5">
    ...
    <div class="col-md-10 p-5 pt-2">
       <h3><i class="fas fa-user mr-2"></i>KAP</h3><hr>
       <table class="table table-striped table-bordered text-center">
          ...
          <?php
            $no =1;
            $tampil = mysqli_query($koneksi, "SELECT * from kap order by Id desc");
          ...
          <tbody>
            <tr>
              ...
              <td><?=$data['Jurusan'];?></td>
              <td><a href="kap.php?hal=edit&Id=<?=$data['Id']?>" class = "bg-warning p-2 text-white rounded" data-toggle="tooltip" title="edit">Edit</a></td>
              <td><a href="kap.php?hal=hapus&Id=<?=$data['Id']?>" class = "bg-danger p-2 text-white rounded" data-toggle="tooltip" title="hapus">Hapus</a></td>
            </tr>
          </tbody>
          <?php endwhile; ?>
      </table>
          ...
          <div class="card-body">
              <form method="post" action="">
                  <div class="form-group">
                      ...
                      <label>Jurusan</label>
                      <input type="text" name="Jabatan" value="<?=@$vJabatan?>" class="form-control" placeholder="Inputkan Jurusan anda" required>
                  </div>
